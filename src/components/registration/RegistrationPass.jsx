 ...
 * Shows a polished confirmation card (the "pass") containing all required
 * information, plus Copy Registration ID and Download Pass actions.
 *
 * Props:
 *   registration : {
 *     id, workshopId, name, email, year, department, registeredAt
 *   }
 *   workshop     : Workshop object (from live workshopList)
 *   sponsor      : Sponsor object | undefined
 *   onRegisterAnother : () => void
 */

// ── Helpers ──────────────────────────────────────────────────────────────────

/** Formats an ISO date string as "15 Aug 2026" */
...
const RegistrationPass = ({ registration, workshop, sponsor, onRegisterAnother }) => {
  ...
  const handleCopy = async () => {
    try {
      ...
      setCopied(true);
      addNotification({
        id: `info-copy-${registration.id}-${Date.now()}`,
        type: "info",
        message: "Registration ID copied to clipboard.",
      });
      // Reset the check icon after 2 s
      setTimeout(() => setCopied(false), 2000);
    } catch {
      // Clipboard API may be unavailable (non-HTTPS dev, denied permission)
      addNotification({
        id: `warn-copy-fail-${Date.now()}`,
        type: "warning",
        message: "Could not copy to clipboard. Please copy the ID manually.",
      });
    }
  };
  ...
  const handleDownload = () => {
    ...
    addNotification({
      id: `info-download-${registration.id}-${Date.now()}`,
      type: "info",
      message: "Registration pass download started.",
    });
  };
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          Your pass has been generated. Save or download it for your records.
        </p>
      </div>
      ...
      >
        ...
        {/* Registration ID row — prominently featured */}
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-dashed border-slate-200 bg-slate-50 px-6 py-4">
          <div>
            ...
            <p
              className="mt-0.5 font-mono text-base font-bold tracking-wide text-slate-900"
              aria-label={`Registration ID: ${registration.id}`}
            >
              {registration.id}
            </p>
          </div>
          ...
          <div className="px-6 py-4">
            ...
            <div className="divide-y divide-slate-100">
              <div className="py-2.5">
                ...
                <p className="mt-0.5 text-xs text-slate-500">{workshop.category}</p>
              </div>
              <PassRow
                icon={CalendarDays}
                label="Date & Time"
                value={`${formatDate(workshop.date)} · ${workshop.time}`}
              />
              <PassRow
                icon={workshop.mode === "Online" ? Wifi : WifiOff}
                label="Mode"
                value={workshop.mode}
              />
              <PassRow icon={MapPin} label="Venue" value={workshop.venue} />
              {sponsor && (
                <PassRow icon={Handshake} label="Sponsor" value={sponsor.name} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-900"
          >
