 ...
 *
 * Collects Full Name, Academic Email, Year of Study, and Department.
 * All fields are required before advancing; inline error messages are shown
 * only after the user attempts to proceed with an empty field.
 *
 * Props:
 *   formData   : { name, email, year, department }
 *   onChange   : (field: string, value: string) => void
 *   onNext     : () => void
 *   errors     : { name?, email?, year?, department? }
 */
...
const inputClass =
  "w-full rounded-lg border px-3 py-2.5 text-sm text-slate-900 shadow-sm transition-colors placeholder:text-slate-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-900";
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Enter your details so we can confirm your registration.
        </p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            className={`${inputClass} ${errors.year ? inputError : inputNormal}`}
          >
          ...
          {errors.year && (
            <p id="reg-year-error" role="alert" className={errorClass}>
              {errors.year}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <select
            ...
            className={`${inputClass} ${errors.department ? inputError : inputNormal}`}
          >
          ...
          {errors.department && (
            <p id="reg-dept-error" role="alert" className={errorClass}>
              {errors.department}
            </p>
          )}
        </div>
