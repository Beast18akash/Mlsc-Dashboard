...
const DetailItem = ({ label, value, children }) => (
  <div>
    ...
    <dd className="mt-1 text-sm font-medium text-slate-900 dark:text-slate-100">{children ?? value ?? "—"}</dd>
  </div>
);
...
const WorkshopDetailsModal = ({ workshop, sponsor, isOpen, onClose }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Workshop Details" size="lg">
      <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <DetailItem label="Seats Filled"  value={`${workshop.seatsFilled} / ${workshop.capacity}`} />
        {workshop.status !== "Completed" && (
          <DetailItem label="Seats Remaining" value={seatsRemaining <= 0 ? "Full" : `${seatsRemaining} remaining`} />
        )}
        <DetailItem label="Status">
          <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${statusStyles[workshop.status] ?? statusStyles.Upcoming}`}>
            {workshop.status}
          </span>
        </DetailItem>
      </dl>
      ...
      {sponsor?.description && (
        <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-700/50">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Sponsor Description</p>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{sponsor.description}</p>
        </div>
      )}
    </Modal>
  );
